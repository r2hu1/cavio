...
import { type MarkdownType, getMarkdown as getMarkdownBase } from "@platejs/ai";
...
export const getMarkdown = (
	...
	try {
		...
		if (
			...
			console.warn(
				"MDX serialization failed, falling back to plain text:",
				error.message,
			);
			...
			switch (options.type) {
				...
				default:
					blocks = editor.api.blocks({ mode: "lowest" }).map(([node]) => node);
					break;
			}
