...
export default function DocumentCard({
	...
	return (
		<DocumentActionContextMenu folderId={folderId} id={id}>
			<div
				title={name}
				className="bg-background dark:shadow-none dark:bg-sidebar cursor-pointer transition hover:border-input hover:shadow-lg shadow-foreground/5 rounded-lg border overflow-hidden"
			>
				...
				<div className="flex gap-3 items-center justify-between p-3 pt-4">
					<Link href={`/folder/${folderId}/${id}`} className="text-sm truncate">
						{name}
					</Link>
				</div>
