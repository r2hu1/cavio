...
export const documentsRouter = createTRPCRouter({
	...
	getAllByFolderId: protectedProcedure
		...
		.query(async ({ input, ctx }) => {
			const [folder] = await db
				.select()
				.from(folders)
				.where(and(eq(folders.id, input.folderId), eq(folders.deleted, false)));
			if (!folder) {
				throw new TRPCError({
					code: "NOT_FOUND",
					message: "Folder not found",
			...
			const fullDocuments = await db
				...
				.from(documents)
				.where(
					and(inArray(documents.id, documentIds), eq(documents.deleted, false)),
				);
	...
	delete: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			await db
				...
				.set({
					documents:
						folder.documents?.filter((docId) => docId !== input.id) ?? [],
				})
				.where(eq(folders.id, input.folderId));
	...
	restoreAll: protectedProcedure.mutation(async ({ ctx }) => {
		...
		const deletedDocumentsList = await db
			.select()
			.from(documents)
			.where(and(eq(documents.userId, userId), eq(documents.deleted, true)));
		...
		for (const [folderId, docIds] of documentsByFolder) {
			...
			if (folder) {
				await db
					...
					.set({
						documents:
							folder.documents?.filter((docId) => !docIds.includes(docId)) ??
							[],
					})
					.where(eq(folders.id, folderId));
			}
